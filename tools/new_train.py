...
def run_training(data_type="bottle",
    ...
    # #check augmentations
    # check_data = MVTecAT(args.data_dir, data_type, transform=train_transform, size=int(size * (1 / min_scale)))
    # dataloader = DataLoader(check_data, batch_size=1, drop_last=True,
    ...
    #     cut1 = cut1.cpu().detach().numpy()
    #     cut1 = np.transpose(cut1, (1, 2, 0))
    #     cut2 = cut2.cpu().detach().numpy()
    #     cut2 = np.transpose(cut2, (1, 2, 0))
    #     plt.figure(0)
    #     plt.subplot(1,3,1)
    #     plt.title("org")
    #     plt.imshow(org)
    #     plt.subplot(1, 3, 2)
    #     plt.title("cutpaste_normal")
    #     plt.imshow(cut1)
    #     plt.subplot(1, 3, 3)
    #     plt.title("cutpaste_scar")
    #     plt.imshow(cut2)
    #     plt.show()
    ...
    for step in range(epochs):
        ...
        if loss < min_loss:
            paddle.save(model.state_dict(), os.path.join(str(model_dir),data_type,
                                                         "best.pdparams"))
            f.write("best:%d.pdparams\n"%epoch)
        if epoch % args.log_interval == 0:
            total_bacth_cost = time.time() - t0

            print("epoch:%d/%d loss:%.4f acc:%.3f avg_reader_cost:%.2f avg_batch_cost:%.2f avg_ips:%.2f lr:%.4f"%(
            epoch,epochs,loss,accuracy,total_reader_cost/(epoch+1),total_bacth_cost/(epoch+1),total_bacth_cost/(epoch+1)/batch_size,optimizer.get_lr()
            ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', default=256*256, type=int,
    ...
    parser.add_argument('--save_interval', default=10000, type=int, help="number of epochs between each model save (default:1000)")
    parser.add_argument('--log_interval', default=1, type=int, help="number of steps between each log print (default:50)")
    parser.add_argument('--minscale', default=0.98, type=float, help="min scale for random crop (default:1)")
